...
async function downloadGame(gameDir, win2) {
  const zipPath = path$1.join(gameDir, "temp_game.zip");
  try {
    ...
    const progressStream = new require$$6.PassThrough();
    progressStream.on("data", (chunk) => {
      downloadedLength += chunk.length;
      if (totalLength > 0) {
        const percentage = downloadedLength / totalLength * 50;
        win2.webContents.send("install-progress", Math.round(percentage));
      }
    });
    await pipeline(
      // @ts-ignore
      require$$6.Readable.fromWeb(response.body),
      progressStream,
      fs$4.createWriteStream(zipPath)
    );
    ...
    win2.webContents.send("install-progress", 100);
    win2.webContents.send("install-finished");
  } catch (error) {
    ...
    if (fs$4.existsSync(zipPath)) {
      await fs$4.promises.unlink(zipPath);
    }
  }
}
function checkGameInstallation(baseDir, releaseType = "latest") {
  const os = process.platform;
  const clientExec = path$1.join(baseDir, "game", releaseType, "Client", os === "win32" ? "HytaleClient.exe" : "HytaleClient");
  const serverExec = path$1.join(baseDir, "game", releaseType, "Server", "HytaleServer.jar");
  const jreExec = path$1.join(baseDir, "jre", "latest", "bin", os === "win32" ? "java.exe" : "java");
  const client = fs$4.existsSync(clientExec);
  const server = fs$4.existsSync(serverExec);
  const jre = fs$4.existsSync(jreExec);
  return {
    client: client ? clientExec : void 0,
    server: server ? serverExec : void 0,
    jre: jre ? jreExec : void 0
  };
}
function launchGame(baseDir, username, win2, releaseType = "latest") {
  ...
  if (!fs$4.existsSync(userDir)) fs$4.mkdirSync(userDir);
  const args = [
    "--app-dir",
    join(dirname(client), ".."),
    "--user-dir",
  ...
  exec(`"${client}" ${args.join(" ")}`, (error, stdout) => {
    ...
    win2.webContents.send("launched");
  });
}
...
ipcMain.handle("get-default-game-directory", () => {
  return path$2.join(app.getPath("userData"), "Hytale");
});
ipcMain.handle("check-game-installation", (_, baseDir) => {
  return checkGameInstallation(baseDir);
});
ipcMain.on("init-install", (e, baseDir) => {
  const installationDir = baseDir;
  if (!fs$5.existsSync(installationDir)) {
    fs$5.mkdirSync(installationDir);
  }
  const win2 = BrowserWindow.fromWebContents(e.sender);
  if (win2) {
    downloadGame(installationDir, win2);
  }
});
ipcMain.on(
  "launch-game",
  (e, baseDir, username, releaseType) => {
    const win2 = BrowserWindow.fromWebContents(e.sender);
    if (win2) {
      launchGame(baseDir, username, win2, releaseType);
    }
  }
);
